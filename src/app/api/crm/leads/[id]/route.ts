...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { error } = await supabase.from('crm_leads').update(update).eq('id', params.id).eq('user_id', user.id)
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })

  if (body.status !== undefined) {
    await supabase.from('crm_activity_logs').insert({ lead_id: params.id, action: 'status_change', details: { status: body.status } })
  }
